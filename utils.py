...
def normalize_frames_mine(frames):
    """
    Convert frames from int8 [0, 255] to float32 [-1, 1].
...
def denormalize_frames_mine(frames):
    """
    Performs the inverse operation of normalize_frames.
...
def get_full_clips(data_dir, num_clips, num_rec_out=1):
    ...
    ep_dirs = np.random.choice(glob.glob(data_dir + '*'), num_clips)
    ...
    for clip_num, ep_dir in enumerate(ep_dirs):
        ...
        ep_frame_paths = sorted(glob.glob(ep_dir + '/*.png'))

        start_index = np.random.choice(len(ep_frame_paths))
        clip_frame_paths = ep_frame_paths[start_index]

        # read in frames

        try:
            frame = imread(clip_frame_paths, mode='RGB')
        except:
            print('image corrupted')
            return np.empty(shape=(0,0))
        norm_frame = normalize_frames_mine(frame)
        clips[clip_num, :c.FULL_HEIGHT, :c.FULL_WIDTH, :] = norm_frame[:c.FULL_HEIGHT, :c.FULL_WIDTH,:]
...
def process_clip():
    ...
    while(clip.size == 0):
        print('Finding better clip:')
        clip = get_full_clips(c.TRAIN_DIR, 1)

    clip = clip[0]
    cropped_clip = np.empty([c.TRAIN_HEIGHT, c.TRAIN_WIDTH, 3] )
...
def get_test_batch(test_batch_size, num_rec_out=1):
    ...
    @return: An array of shape:
             ...
             A batch of frame sequences with values normalized in range [-1, 1].
    """
    return get_full_clips(c.TRAIN_DIR, test_batch_size, num_rec_out=num_rec_out)
